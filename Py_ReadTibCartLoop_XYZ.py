...
ltcfile = open(wdir+'GenericFiles\\Nodes_LTIBIACARTILAGE.data')
ltclist = ltcfile.readlines()
ltcfile.close()
mtcfile = open(wdir+'GenericFiles\\Nodes_MTIBIACARTILAGE.data')
...
for lrptfilename in lrptfilelist:
    ...
    outrptfile = open(lrptfilename[:-4]+'.data','w')
...
for mrptfilename in mrptfilelist:
    ...
    outrptfile = open(mrptfilename[:-4]+'.data','w')
    outrptfile.writelines(output)
    outrptfile.close()
